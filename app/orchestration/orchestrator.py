...
class AIVAROrchestrator:
    ...
    def run(self,url,credentials=None,prd_text=None,intent=None,parallel=True):
        obs=self.explorer.explore(url); plan=self.planner.plan(obs,prd_text=prd_text,intent=intent); cov=self.coverage.evaluate(plan); attempts=0
        ...
        if parallel and len(gen.tests)>1:
            with ThreadPoolExecutor(max_workers=min(4,len(gen.tests))) as pool:
                results=list(pool.map(self.executor.run_test, gen.tests))
        else:
        ...
        return QualityReport(run_id=uuid.uuid4().hex[:12],application_url=url,total_planned=len(plan.scenarios),total_generated=len(gen.tests),total_executed=len(results),passed=passed,healed=healed,failed=failed,escalated=esc,blocked=blocked,coverage_score=cov.score,coverage_gaps=cov.gaps,healer_actions=actions,risk=risk,scenarios=plan.scenarios,results=results,prd_gap=gap,intent=intent,summary=f"Planned {len(plan.scenarios)}, generated {len(gen.tests)}, executed {len(results)}, passed {passed}, healed {healed}, failed {failed}, escalated {esc}, blocked {blocked}; coverage {cov.score:.0%}.")
